...
class path_planner:
    ...
    def update(self, map, state):
        ...
        if planner_flag == 1:
            ...
            wpp_start = np.array([state.n, state.e, -state.h, state.Va])

            temp = np.linalg.norm( np.array([state.n, state.e, -state.h]) - \
                                   np.array([map.city_width, map.city_width, -state.h]) )
            if temp == 0:
                wpp_end = np.array([0, 0, -state.h, Va])
            else:
                wpp_end = np.array([map.city_width, map.city_width, -state.h, Va])
